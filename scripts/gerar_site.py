...
function renderGlobalKpis(){{
  ...
  // Sparkline — taxa por dia confirmado
  ...
  const taxasPorDia = diasConf.map(([d,v])=>{{
    const s=v.resultado_stats||{{}};
    let ta=0,te=0;
    Object.values(s).forEach(x=>{{ta+=x.acertos||0;te+=x.erros||0;}});
    return ta+te>0?Math.round(ta/(ta+te)*100):null;
  }}).filter(x=>x!==null);

  // Gerar sparkline SVG
  let sparkHtml='';
  if(taxasPorDia.length>0){{
    const max=100, min=0, w=120, h=28, pad=2;
    const bw=Math.max(4, Math.floor((w-pad*(taxasPorDia.length-1))/taxasPorDia.length));
    const bars=taxasPorDia.map((t,i)=>{{
      const bh=Math.max(2,Math.round((t/max)*(h-4)));
      const x=i*(bw+pad);
      const y=h-bh;
      const col=t>=80?'#22c55e':t>=60?'#eab308':'#ef4444';
      return`<rect x="${{x}}" y="${{y}}" width="${{bw}}" height="${{bh}}" rx="1" fill="${{col}}" opacity="0.85">
        <title>${{diasConf[i][0].slice(0,5)}} · ${{t}}%</title>
      </rect>`;
    }}).join('');
    const totalW=(bw+pad)*taxasPorDia.length-pad;
    sparkHtml=`<div style="display:flex;flex-direction:column;align-items:flex-start;gap:2px;margin-left:8px;border-left:1px solid var(--border);padding-left:10px">
      <span style="font-size:9px;font-weight:700;color:var(--muted);letter-spacing:.8px;text-transform:uppercase">Acerto por dia</span>
      <svg width="${{totalW}}" height="${{h}}" viewBox="0 0 ${{totalW}} ${{h}}">${{bars}}</svg>
    </div>`;
  }}

  document.getElementById('global-kpis').innerHTML=`
    <div style="display:flex;flex-direction:column;gap:4px">
      <span style="font-size:9px;font-weight:700;color:var(--muted);letter-spacing:1px;text-transform:uppercase">Desempenho das Melhores Previsões</span>
